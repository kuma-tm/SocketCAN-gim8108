...
namespace scpp
{
    ...
    SocketCanStatus SocketCan::open(const std::string & can_interface, int32_t read_timeout_ms, SocketMode mode)
    {
        ...
        {
            /* check if the frame fits into the CAN netdevice インターフェースのMTUを取得*/
            ...
            /* interface is ok - try to switch the socket into CAN FD mode ソケットをCAN FD対応モードに*/
...
// LINUX
        ...
        tv.tv_usec = m_read_timeout_ms * 1000;  // Not init'ing this can cause strange errors  SO_RCVTIMEOをセット→read()のブロッキング時間をms単位で設定
        setsockopt(m_socket, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv,sizeof(struct timeval));
		
		//これでCANソケットがcan0にバインドされる
    ...
    SocketCanStatus SocketCan::write(const CanFrame & msg)
    {
        ...
        memset(&frame, 0, sizeof(frame)); /* init CAN FD frame, e.g. LEN = 0   初期化 */
        //convert CanFrame to canfd_frame CanFrame → canfd_frameへコピー
        frame.can_id = msg.id;
        frame.len = msg.len;
        frame.flags = msg.flags;
        memcpy(frame.data, msg.data, msg.len);
		
		// FDモードのときはDLCに合わせたlenに補正（FDは関係ないが）
        if (m_socket_mode == MODE_CANFD_MTU)
        {
            /* ensure discrete CAN FD length values 0..8, 12, 16, 20, 24, 32, 64 */
            frame.len = can_dlc2len(can_len2dlc(frame.len));
        }
        /* send frame  write()で送信*/
    ...
    {
        close();
        // デストラクタでclose()を呼ぶので，オブジェクトがスコープを抜けたときにソケットも自動的に閉じる設計
    }
}
